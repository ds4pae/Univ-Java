...
public class LoginPanel extends JPanel implements ActionListener {
	...
	public JButton register_BT = new JButton("Register");
	...
	public LoginPanel(Login login) {
		...
		userlist.add(new UserInfo("Brother", "1234", "01012345678"));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == login_BT) {
			for(UserInfo i : userlist) {
				if ((login_ID_TF.getText().equals(i.getID())) && password.equals(i.getPW()) ) {
					System.out.println("Correct ID && PW");
					RadioButtonGallery rbg = new RadioButtonGallery();
				}
				else {
					loginCount++;
					JOptionPane.showMessageDialog(null,"IncorrectID && PW , 실패 횟수 : "+ loginCount);
				}
			}
			if(loginCount >= 3) {
		...
		if (e.getSource() == register_BT) {
			phoneNum = JOptionPane.showInputDialog(null, "전화번호를 입력해주세요 ('-' 제외)");
			userlist.add(new UserInfo(login_ID_TF.getText(), password, phoneNum));
		}
	}
}